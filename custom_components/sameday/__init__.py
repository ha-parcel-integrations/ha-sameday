"""Sameday parcel tracker custom component for Home Assistant."""
...
@dataclass
class SamedayData:
    """Runtime data attached to the Sameday config entry."""

    client: SamedayApiClient
    coordinator: SamedayCoordinator


type SamedayConfigEntry = ConfigEntry[SamedayData]


async def async_setup_entry(hass: HomeAssistant, entry: SamedayConfigEntry) -> bool:
    """Set up Sameday from a config entry."""
    # No auth: Sameday tracking is public, so the HA-managed session is fine.
    # The country (host TLD) is chosen once at setup and fixed for the entry.
    country = entry.data.get(CONF_COUNTRY, DEFAULT_COUNTRY)
    client = SamedayApiClient(async_get_clientsession(hass), country)
    ...
    entry.runtime_data = SamedayData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: SamedayConfigEntry) -> bool:
    """Unload the Sameday config entry."""
