"""Calendar platform for the Sameday parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Sameday deliveries calendar from a config entry."""
    async_add_entities([SamedayDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class SamedayDeliveriesCalendar(CoordinatorEntity[SamedayCoordinator], CalendarEntity):
    """A read-only calendar of expected Sameday deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Sameday parcel")
            description_parts = [
