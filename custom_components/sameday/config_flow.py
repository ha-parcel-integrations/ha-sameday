"""Config flow for the Sameday parcel tracker integration."""
...
class SamedayConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the Sameday integration."""
    ...
    ) -> SamedayOptionsFlowHandler:
        """Return the options flow handler."""
        return SamedayOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the Sameday hub — single instance, country picked at setup.

        Tracking is keyed on the AWB alone (no account, no postal code), so the
        only thing to ask is which national Sameday host to use (ro/hu/bg): the
        same API runs on all three and the country fixes the host for the
        entry's lifetime. Parcels are added afterwards via the options flow, the
        ``sameday.track_parcel`` service or a dashboard button.
