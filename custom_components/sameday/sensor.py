"""Sensor platform for the Sameday parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: SamedayConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Sameday sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        SamedayIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        SamedayAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            SamedayParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(SamedayNextDeliverySensor(coordinator, entry))
    entities.append(SamedayDeliveredParcelsSensor(coordinator, entry))
    entities.append(SamedayLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class SamedayIncomingParcelsSensor(
    CoordinatorEntity[SamedayCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: SamedayCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                SamedayParcelSensor(self.coordinator, self._entry, barcode)
...
class SamedayAwaitingPickupSensor(
    CoordinatorEntity[SamedayCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: SamedayCoordinator, entry: ConfigEntry) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class SamedayParcelSensor(CoordinatorEntity[SamedayCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Sameday parcel."""
    ...
    def __init__(
        self, coordinator: SamedayCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class SamedayNextDeliverySensor(
    CoordinatorEntity[SamedayCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: SamedayCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class SamedayDeliveredParcelsSensor(
    CoordinatorEntity[SamedayCoordinator], SensorEntity
):
    """Recently delivered tracked Sameday parcels."""
...
class SamedayLastUpdateSensor(
    CoordinatorEntity[SamedayCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Sameday was last polled successfully."""
