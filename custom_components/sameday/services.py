"""Services for the Sameday parcel tracker integration.

`sameday.track_parcel` / `sameday.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Sameday hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Sameday is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Sameday services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Sameday tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Sameday services (single-entry integration, so on unload)."""
