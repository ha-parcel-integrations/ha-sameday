# Automatic parcel tracking from e-mail (IMAP → Sameday)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Sameday tracking code, and register it with `sameday.track_parcel` — fully automatic, no extra custom component required.

Sameday is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `sameday.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ sameday.track_parcel
                                     │
...
- This integration, with the `sameday.track_parcel` action available (field `tracking_code`).
...
Sameday AWBs are long numeric strings with **no distinctive prefix**, so the regex anchors on the label *next to* the number instead of on the number itself:

```
(?:AWB|awb|expeditie)[^0-9]{0,12}([0-9]{6,24})
```

The integration itself accepts any AWB matching `^[A-Z0-9]{6,24}$`, but matching *that* broadly against e-mail text would hit every order number in the mail — so the example only picks up a number sitting near the word **AWB** (Sameday's own label). **If your mails use a different word for it, add it to the alternation** (`(?:AWB|awb|expeditie|YOURWORD)`); the AI fallback will surface anything the regex misses in the meantime.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@samedayshipping.com","subject":"Your parcel is on its way",
       "text":"Your AWB 0812345678 is on its way","initial":true,"folder":"INBOX","username":"test"}'
```

Then `sameday.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
