"""Tests for the Sameday coordinator: fetching, caching and events.
...
from custom_components.sameday.coordinator import SamedayCoordinator

from .payloads import ACTIVE_CODE, DELIVERED_CODE, active_sample, delivered_sample

OTHER_CODE = "2SDAY0008888"
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    """An active parcel whose current status is in-transit (statusStateId 7)."""
    sample = active_sample(code)
    sample["awbHistory"][-1]["statusStateId"] = "7"
    sample["awbHistory"][-1]["statusState"] = "In tranzit"
    return sample
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = SamedayApiError("HTTP 500")
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    Sameday's public timeline exposes no ETA, so ``planned_from`` is always
    ``None`` on a real parcel and this event never fires in practice. The branch
    is shared suite code, so drive ``_fire_change_events`` directly with a
    synthetic window shift rather than pretending Sameday reports one.
    """
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    coordinator = SamedayCoordinator(hass, AsyncMock(), entry)
