"""Tests for Sameday sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.sameday.const import ParcelStatus
from custom_components.sameday.sensor import (
    SamedayAwaitingPickupSensor,
    SamedayDeliveredParcelsSensor,
    SamedayIncomingParcelsSensor,
    SamedayLastUpdateSensor,
    SamedayNextDeliverySensor,
    SamedayParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = SamedayIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = SamedayParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = SamedayParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = SamedayNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = SamedayNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = SamedayDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = SamedayLastUpdateSensor(coordinator, _entry())
